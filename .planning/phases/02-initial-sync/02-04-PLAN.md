...
Purpose: Users need visual feedback during sync (XFER-02), confirmation when complete (XFER-03), and a warning before overwrite (XFER-04). This plan delivers those UI requirements and wires everything together.

Output: SyncProgress component, OverwriteConfirmDialog component, App integration with SyncProvider.
</objective>
...
- Spinner from @/components/ui/spinner (if available) or use CSS animation
- cn from @/lib/utils
...
<task type="auto">
  <name>Task 3: Integrate SyncProvider and add toast notification</name>
...
3. Add effect to show toast on sync completion:
```tsx
const { syncState, commitSync } = useSync();
...
4. Add "Sync Project" button when connected:
...
Visually: "Sync Project" button appears when connected. Toast shows on sync complete.
  </verify>
  <done>
App wrapped with SyncProvider. SyncProgress and OverwriteConfirmDialog render globally. Header shows "Sync Project" button when connected and toast on completion.
  </done>
</task>
...
- [ ] Header.tsx has "Sync Project" button and completion toast
- [ ] Toast notification fires when sync completes (sonner)
</verification>
...
<checkpoint type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete initial sync UI: progress bar, overwrite warning dialog, sync button, completion toast</what-built>
  <how-to-verify>
1. Open app in two browser windows
2. Establish P2P connection between them (use ConnectionDialog)
3. In Window 1 (editor), click "Sync Project" button in header
4. Verify Window 2 shows OverwriteConfirmDialog
5. Click "Accept" in Window 2
6. Watch progress bar appear in both windows
7. When complete, verify toast shows "Project synced successfully!"
