...
6. Add to SyncContextValue:
  ...
  <verify>npx tsc --noEmit (no type errors)</verify>
  <done>Operation handler routes all five operation types, isApplyingRemoteRef prevents re-broadcast, audio changes handled via pendingAudioOps, getConnection/getAudioTransfer exposed and callable from consuming hooks</done>
</task>
...
6. getConnection and getAudioTransfer are exported via context value
</verification>
...
- getConnection/getAudioTransfer accessible to consuming hooks
</success_criteria>
