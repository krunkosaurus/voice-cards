...
files_modified:
  ...
  - client/src/components/CardListItem.tsx
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "client/src/components/CardListItem.tsx"
...
Output: useSyncedActions hook; Home.tsx and CardListItem.tsx use synced actions.
</objective>
...
@.planning/phases/03-real-time-sync/03-02-SUMMARY.md
...
<task type="auto">
  <name>Task 3: Update CardListItem.tsx to use synced actions</name>
  <files>client/src/components/CardListItem.tsx</files>
  <action>
Update CardListItem.tsx to use useSyncedActions for card updates.
...
Note: The component may have inline handlers or use callbacks passed from parent. Follow the existing pattern - if updates are done here, use synced actions. If done in parent, the parent should use synced actions.
  ...
  <verify>npx tsc --noEmit (no type errors)</verify>
  <done>CardListItem.tsx uses synced actions for card updates and audio changes</done>
</task>
...
4. CardListItem.tsx imports and uses synced actions for updates
