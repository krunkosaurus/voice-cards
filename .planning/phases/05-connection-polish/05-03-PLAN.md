...
must_haves:
  truths:
    ...
    - "Context detects connection loss via heartbeat timeout"
    - "Context shows brief reconnecting state before requiring manual intervention"
    - "Context handles peer disconnect message without auto-reconnect attempt"
    - "Context exposes gracefulDisconnect for UI disconnect button"
  artifacts:
